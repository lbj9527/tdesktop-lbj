...
namespace Window
{
  ...
  FiltersMenu::FiltersMenu(
      ...
      : _session(session), _parent(parent), _outer(_parent), _menu(&_outer, TextWithEntities(), st::windowFiltersMainMenu), _scroll(&_outer), _container(
  ...
  void FiltersMenu::setup()
  {
    ...
    _outer.show();
    _outer.paintRequest() | rpl::start_with_next([=](QRect clip)
                                                 {
		auto p = QPainter(&_outer);
		p.setPen(Qt::NoPen);
		p.setBrush(st::windowFiltersButton.textBg);
		p.drawRect(clip); }, _outer.lifetime());

    _parent->heightValue() | rpl::start_with_next([=](int height)
                                                  {
		...
		_container->move(0, 0); }, _outer.lifetime());
    ...
    rpl::combine(
        ...
        std::move(premium)) |
        rpl::start_with_next([=]
                             { refresh(); }, _outer.lifetime());

    _activeFilterId = _session->activeChatsFilterCurrent();
    _session->activeChatsFilter() | rpl::filter([=](FilterId id)
		...
		_reorder->finishReordering(); }, _outer.lifetime());
  ...
  void FiltersMenu::setupMainMenuIcon()
  {
    OtherAccountsUnreadState(
		...
		_menu.setIconOverride(icon, icon); }, _outer.lifetime());
  }
  ...
  {
    _list = object_ptr<Ui::VerticalLayout>::fromRaw(
        _container->add(object_ptr<Ui::VerticalLayout>(_container.get())));
    ...
    _transhub->setClickedCallback([=]
		...
		Boxes::ShowTransHubBox(_session); });
    _reorder = std::make_unique<Ui::VerticalLayoutReorder>(_list, &_scroll);

    _reorder->updates() | rpl::start_with_next([=](Ui::VerticalLayoutReorder::Single data)
                                               {
      ...
      bool toBeginning)
  {
    ...
    if (id >= 0)
    {
      ...
      raw->events() | rpl::filter([=](not_null<QEvent *> e)
                                  { return ((e->type() == QEvent::ContextMenu) && (id >= 0)) || e->type() == QEvent::DragEnter || e->type() == QEvent::DragMove || e->type() == QEvent::DragLeave; }) |
  ...
  void FiltersMenu::openFiltersSettings()
  {
    ...
    else if (!_waitingSuggested)
    {
      ...
      filters->suggestedUpdated() | rpl::take(1) | rpl::start_with_next([=]
                                                                        { _session->showSettings(Settings::Folders::Id()); }, _outer.lifetime());
    }
  }
